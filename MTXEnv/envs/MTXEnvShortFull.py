from .MTXEnvShortSimple import *

class MTXEnvShortFull(MTXEnvShortSimple):
    def step(self, action):
        ...
        if self.index >= len(self.time_arr) - 1 or end_asset <= 0:
            done = True
    ...
    def process_trade(self, action):
        ...
        transaction_fee = self.feat_arr[self.index, self.col_index['close']] * self.POINT_VALUE * self.TRANSACTION_FEE_PERCENT
        reward = 0
        ...
        if action < 0: # sell    
            ...
            for _ in range(abs(action)):
                ...
                self.balance += (bid_price - self.feat_arr[self.index, self.col_index['close']]) * self.POINT_VALUE
                reward += bid_price - self.feat_arr[self.index, self.col_index['close']]
                if bid_price > self.feat_arr[self.index, self.col_index['close']]:
                    self.win += 1
            self.shares += abs(action)
    ...
    def valid_action_mask(self):
        ...
        # Disable buy actions beyond available amount
        actions_mask[101+abs(self.shares):201] = False
        
        # Disable sell actions beyond available amount
        actions_mask[0:100-available_amount] = False    
        return actions_mask 
    ...
    def finalize_episode(self, asset):
        ...
        self.balance -= (self.feat_arr[self.index, self.col_index['close']] * self.POINT_VALUE * self.TRANSACTION_FEE_PERCENT) * abs(self.shares)
