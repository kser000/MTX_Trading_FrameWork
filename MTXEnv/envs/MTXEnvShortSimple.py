...
class MTXEnvShortSimple(MTXEnvSyn):
    ...
    def calculate_asset(self):
        ...
        for i in range(int(abs(self.shares))):
            if self.shares >= 0:
                ...
                asset +=  self.POINT_VALUE * (self.last_bid[i] - self.feat_arr[self.index, self.col_index['close']])
        return asset
    ...
    def valid_action_mask(self):
        ...
        if not self.is_tradable():  # only hold(100) valid
            actions_mask[0:100] = False
            actions_mask[101:201] = False
        elif self.shares >= 0:  # only hold(100) and sell actions valid
            # Only allow sell action 1 (action 99)
            actions_mask[0:99] = False
            actions_mask[101:201] = False
        elif self.shares < 0:  # only hold(100) and buy actions valid
            actions_mask[0:100] = False
            # Only allow buy action 1 (action 101)
